SELECT bpm_mat_ei.id, pl.user_id, bpm_mat_ei.name, SUM( ( required_qtt*COALESCE( bmo.percent_modification_value, 1 ) )*runs_count ) - COALESCE( ba.quantity, 0 ) qtt_to_buy,
                                                   ( SUM( ( required_qtt*COALESCE( bmo.percent_modification_value, 1 ) )*runs_count ) - COALESCE( ba.quantity, 0 ) )*bpm_mat_ei.cost total_cost,
                                                   ( SUM( ( required_qtt*COALESCE( bmo.percent_modification_value, 1 ) )*runs_count ) - COALESCE( ba.quantity, 0 ) )*bpm_mat_ei.volume required_volume
...
HAVING SUM( ( required_qtt*COALESCE( bmo.percent_modification_value, 1 ) )*runs_count ) - COALESCE( ba.quantity, 0 ) > 0;
