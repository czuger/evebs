SELECT bpm_mat_ei.id, pl.user_id, bpm_mat_ei.name eve_item_name, bpm_mat_ei.id eve_item_id,
...
FROM production_lists pl
  ...
  LATERAL (SELECT CEIL( required_qtt*runs_count*COALESCE( bmo.percent_modification_value, 1 ) ) AS raw_qtt) qtt_comp
WHERE runs_count > 0
