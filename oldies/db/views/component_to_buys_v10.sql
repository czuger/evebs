...
-- This is not used anymore. See components_to_buy instead.
-- Anyway, new version would be :
...
FROM production_lists pl
  ...
  LATERAL (
  SELECT required_qtt*runs_count*COALESCE( bmo.percent_modification_value, 1 ) * constants.f_value AS raw_qtt
  FROM constants
  ) qtt_comp
WHERE runs_count IS NOT NULL
GROUP BY bpm_mat_ei.id, pl.user_id, bpm_mat_ei.name, COALESCE( ba.quantity, 0 )
