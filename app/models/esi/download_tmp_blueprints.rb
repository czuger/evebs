class Esi::DownloadTmpBlueprints < Esi::Download
  ...
  def update
    ActiveRecord::Base.transaction do
      ...
      ActiveRecord::Base.connection.execute('TRUNCATE tmp_download_blueprints RESTART IDENTITY')
      load_blueprints
    end
  end
  ...
  def load_blueprints
    ...
    blueprints.each do |blueprint|
      ...
      records << TmpDownloadBlueprint.new( name:
      )
  ...
  def process_materials( blueprint )
    @materials.each do |material|
      ...
      comp = TmpDownloadBlueprint.find_by_cpp_eve_item_id(t_id )
      ...
      comp = TmpDownloadBlueprint.where(cpp_eve_item_id: t_id ).first_or_initialize
      comp.name = local_page['name']
      comp.volume = local_page['volume']
      comp.save!

      ar_material = BlueprintMaterial.where(blueprint_id: blueprint.id, eve_items_id: comp.id ).first_or_initialize
