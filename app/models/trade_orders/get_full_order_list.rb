module TradeOrders::GetFullOrderList
  ...
  def get_full_order_list( user, full_orders_list )
    ...
    full_orders_list.each do |order|
      puts "Order received #{order.typeID}, #{order.stationID}, #{order.price}" unless Rails.env == 'test'
      ...
      if trade_hub_id && eve_item_id
        ...
        if to
          ...
          puts "Found #{to.inspect}, updating" unless Rails.env == 'test'
          to.update_attributes( { new_order: true, price: order.price } )
        else
          # Sinon on en cree un
          puts "Order not found - creating a new one" unless Rails.env == 'test'
          TradeOrder.create!( user: user, eve_item_id: eve_item_id, trade_hub_id: trade_hub_id, new_order: true, price: order.price )
        end
      else
        STDERR.puts "#{Time.now} - TradeOrder.get_trade_orders - Unable to find eve item for id #{order.typeID}" unless eve_item_id unless Rails.env == 'test'
        STDERR.puts "#{Time.now} - TradeOrder.get_trade_orders - Unable to find trade hub for station #{order.stationID}" unless trade_hub_id unless Rails.env == 'test'
      end
    end
