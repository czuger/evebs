class ChooseTradeHubsController < ApplicationController
  ...
  def edit
    @user = current_user
    @inner_trade_hubs_1 = TradeHub.where( inner: true ).where.not( region: nil ).limit( 16 ).order(:region_id, :name).to_a
    @inner_trade_hubs_2 = TradeHub.where( inner: true ).where.not( region: nil ).offset( 16 ).order(:region_id, :name).to_a
